...
class Visualer(object):
    ...
    def __init__(self,save_dir,visual_frequency,visual_batchs):
        self.save_dir=save_dir
        self.visual_batchs=visual_batchs
    ...
    def write(self,epoch,cnt_batch,batch_list,child_dir,file_name_batch):


        if epoch%self.visual_frequency!=0:
            return
        if cnt_batch>self.visual_batchs:
            return
        file_names=[ file_name_batch[i] for i  in range(len(file_name_batch))]
        save_dir=os.path.join(self.save_dir,child_dir)
        visualization(batch_list,file_names,save_dir)
